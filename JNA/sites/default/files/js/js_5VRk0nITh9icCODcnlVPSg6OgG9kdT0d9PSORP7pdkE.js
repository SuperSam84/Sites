...
(function ($, Drupal, drupalSettings) {
    Drupal.behaviors.sp = {
        attach: function (context, settings) {
            ...
            $('.sp-ui-layer a').on('click', function (e) {
                e.preventDefault();
            });
            $('.sp-ui-slider-settings .form-item label, .sp-ui-slider-info .form-item label').hover(function () {
                $this = $(this);
                var $tip = $this.parents('.form-item').find('div.description');
                var $tip_width = $tip.width();
                $tip.css('left', -($tip_width + 35));
                $tip.css('top', -($tip.height() / 2));
                $tip.show();
            ...
            $('.sp-ui-layers-links a').on('click', function () {
                $('.sp-ui-layers-links a').removeClass('active');
                $(this).addClass('active');
                $('.sp-ui-layers-drag .sp-ui-layer').removeClass('sp-ui-layer-active');
                var layer_id = $(this).attr('data-layer-id');
                if ($('.sp-ui-layers-drag ').find('.sp-ui-layer[data-layer-id="' + layer_id + '"]').length) {
                    $('.sp-ui-layers-drag .sp-ui-layer[data-layer-id="' + layer_id + '"]').addClass('sp-ui-layer-active');
                }

            });

            var $sp_ui_dragging = false;

            $('.sp-ui-layers-drag .sp-ui-layer').draggable({
                start: function (event, ui) {
                    $sp_ui_dragging = true;
                    var $layer_selected = $(this);
                    var $layer_selected_id = $layer_selected.attr('data-layer-id');

                    if (!$('form[data-layer-id="' + $layer_selected_id + '"]').length) {
                        $('.sp-ui-layers-links').find("a[data-layer-id='" + $layer_selected.attr('data-layer-id') + "']").click();
                    }
                ...
                stop: function (event, ui) {
                    $sp_ui_dragging = false;
                    var autoRightBottom = false;
                    var $this = $(this);
                    var $layer = $this;
                    var $layer_id = $layer.attr('data-layer-id');
                    var $layer_settings = $('.sp-layer-settings-form').find('form[data-layer-id="' + $layer_id + '"]');

                    var position = $layer.attr('data-position').toLowerCase(),
                        horizontalPosition,
                        verticalPosition,
                        layerLeft = parseInt(ui.position.left, 10),
                        layerTop = parseInt(ui.position.top, 10),
                        layerWidth = parseInt($layer.css("width"), 10),
                        layerHeight = parseInt($layer.css('height'), 10),
                        viewportWidth = parseInt($('.sp-ui-layers-drag-wrapper').css('width'), 10),
                        viewportHeight = parseInt($('.sp-ui-layers-drag-wrapper').css('height'), 10);


                    if ($('select[name="settings[appearance][data-position]"]').length) {
                        position = $('select[name="settings[appearance][data-position]"]').val().toLowerCase();
                    }
                    ...
                    if (horizontalPosition === 'left') {
                        if ($layer_settings.find('input[name="settings[appearance][data-horizontal]"]').length) {
                            $layer_settings.find('input[name="settings[appearance][data-horizontal]"]').val(layerLeft);
                        }

                    } else if (horizontalPosition === 'right') {
                        var right = viewportWidth - layerLeft - layerWidth;
                        if ($layer_settings.find('input[name="settings[appearance][data-horizontal]"]').length) {
                            $layer_settings.find('input[name="settings[appearance][data-horizontal]"]').val(right);
                        }

                        //$layer.css({'left': 'auto', 'right': right}); // noted

                    } else {
                        var horizontalCenter = -( viewportWidth - 2 * layerLeft - layerWidth );
                        if ($('input[name="settings[appearance][data-horizontal]"]').length) {
                            $layer_settings.find('input[name="settings[appearance][data-horizontal]"]').val(horizontalCenter);
                        }
                        // $layer.css({'left': horizontalCenter, 'right': 0});
                    }

                    if (verticalPosition === 'top') {
                        if ($layer_settings.find('input[name="settings[appearance][data-vertical]"]').length) {
                            $layer_settings.find('input[name="settings[appearance][data-vertical]"]').val(layerTop);
                        }
                    } else if (verticalPosition === 'bottom') {
                        var bottom = viewportHeight - layerTop - layerHeight;
                        if ($layer_settings.find('input[name="settings[appearance][data-vertical]"]').length) {
                            $layer_settings.find('input[name="settings[appearance][data-vertical]"]').val(bottom);
                        }
                        // $layer.css({'top': 'auto', 'bottom': bottom});
                    } else {
                        var verticalCenter = -( viewportHeight - 2 * layerTop - layerHeight );
                        if ($layer_settings.find('input[name="settings[appearance][data-vertical]"]').length) {
                            $layer_settings.find('input[name="settings[appearance][data-vertical]"]').val(verticalCenter);
                        }
                        //$layer.css({'top': verticalCenter, 'bottom': 0});
                    }


                    $layer_settings.find('.sp-layer-form-btn-submit').mousedown();
                },
            });


            // position change, width, height, change...

            if ($('select[name="settings[appearance][data-position]"]').length) {
                $('select[name="settings[appearance][data-position]"], input[name="settings[appearance][data-horizontal]"],input[name="settings[appearance][data-vertical]"],input[name="settings[appearance][data-width]"], input[name="settings[appearance][data-height]"]').on('change', function () {

                    if (!$sp_ui_dragging) {
                        var $this = $(this);
                        var $layer_id = $this.parents('form').attr('data-layer-id');
                        var $layer = $('.sp-ui-layers-drag').find('.sp-ui-layer[data-layer-id="' + $layer_id + '"]');


                        var position = $('select[name="settings[appearance][data-position]"]').val().toLowerCase(),
                            horizontalPosition,
                            verticalPosition,
                            horizontalValue = $('input[name="settings[appearance][data-horizontal]"]').val(),
                            verticalValue = $('input[name="settings[appearance][data-vertical]"]').val(),
                            width = $('input[name="settings[appearance][data-width]"]').val(),
                            height = $('input[name="settings[appearance][data-height]"]').val(),
                            suffix,
                            layerWidth = parseInt($layer.css('width'), 10),
                            layerHeight = parseInt($layer.css('height'), 10);
                        ...
                        if (horizontalPosition === 'center') {
                            $layer.css({
                                ...
                                'marginLeft': 'auto',
                                'marginRight': 'auto',
                                'left': horizontalValue + suffix,
                                'right': 0
                            });
                        } else {
                            $layer.css(horizontalPosition, horizontalValue + suffix);
                        }
                        ...
                        if (verticalPosition === 'center') {
                            $layer.css({
                                ...
                                'marginTop': 'auto',
                                'marginBottom': 'auto',
                                'top': verticalValue + suffix,
                                'bottom': 0
                            });
                        } else {
                            $layer.css(verticalPosition, verticalValue + suffix);
                        }
                    }
            ...
            if ($('select[name="settings[appearance][preset_styles][]"]').length) {
                $('select[name="settings[appearance][preset_styles][]"], input[name="settings[appearance][custom_class]"]', context).on('change', function () {
                    var $layer_id = $(this).parents('form').attr('data-layer-id');
                    var $layer = $('.sp-ui-layers-drag').find('.sp-ui-layer[data-layer-id="' + $layer_id + '"]');

                    var classes = '',
                        selectedStyles = '',
                        customClass = $('input[name="settings[appearance][custom_class]"]').val();

                    $('select[name="settings[appearance][preset_styles][]"]').find('option[selected="selected"]').each(function () {
                        selectedStyles += ' ' + $(this).val();
                    });
                    classes += selectedStyles;
                    classes += customClass !== '' ? ' ' + customClass : '';

                    if ($layer.hasClass('sp-layer')) {
                        $layer.attr('class', 'sp-ui-layer sp-layer' + classes);
                    } else {
                        $layer.find('.sp-layer').attr('class', 'sp-ui-layer sp-layer' + classes);
                    }
            ...
            if ($('.sp-layer-settings-form select[multiple="multiple"]').length) {
                $('.sp-layer-settings-form').find('select[multiple="multiple"]', context).multiCheck({width: 120});
            }


            // sortable

            $('.sp-ui-layers-links').sortable({
                ...
                start: function () {
                    ...
                    $('.sp-ui-layers-links').addClass('sp-ui-sorting-started');
                },
                update: function (event, ui) {
                    ...
                    $this.find('a').each(function () {
                        var $layer_link = $(this);
                        var $element = [];
                        $element.push($layer_link.attr('data-layer-id'));
                        var parent = $layer_link.attr('data-slide-id');
                    ...
                    var sortData = {elements: $elements};
                    var sortURL = drupalSettings.path.baseUrl + 'sp/layer/sortable/' + $sid;
                    $.ajax({
                        data: sortData,
                        type: 'POST',
                        url: sortURL
                ...
                stop: function (event, ui) {
                    $('.sp-ui-layers-links').removeClass('builder-ui-sorting-started');
                }
            });


            // Javascript place here...
