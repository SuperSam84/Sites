...
(function ($, Drupal, drupalSettings) {
    Drupal.behaviors.sp_slider = {
        attach: function (context, settings) {
            //console.log(drupalSettings);
            if (!($.type(settings.sp) === 'undefined') && !($.type(settings.sp) === "null")) {
                $.each(settings.sp, function (sp_name, sp_settings) {
                    if ($('#sp-slider-' + sp_name).length) {
                        $('#sp-slider-' + sp_name).sliderPro(sp_settings);
                    }
